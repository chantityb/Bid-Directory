...
bid_radix theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of bid_radix theme and run the following commands:
`nvm use`
...
`drush en bid_radix -y`
`drush config-set system.theme default bid_radix -y`
